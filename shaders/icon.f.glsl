...
vec4 sample_texture(vec2 uv, float fmid){
    vec4 color;
    if(fmid < 0.5) color = texture(image0, uv);
    else if(fmid > 0.5 && fmid < 1.5) color = texture(image1, uv);
    else if(fmid > 1.5 && fmid < 2.5) color = texture(image2, uv);
    else if(fmid > 2.5 && fmid < 3.5) color = texture(image3, uv);
    else if(fmid > 3.5 && fmid < 4.5) color = texture(image4, uv);
    else if(fmid > 4.5 && fmid < 5.5) color = texture(image5, uv);
    else if(fmid > 5.5 && fmid < 6.5) color = texture(image6, uv);
    else if(fmid > 6.5 && fmid < 7.5) color = texture(image7, uv);
